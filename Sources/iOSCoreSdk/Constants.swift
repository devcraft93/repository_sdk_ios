...
struct Constants {
  ...
  static let itemsPerPage:Int  = 1000 // Current, paging from BE is not supported.
}
...
struct USER_DEFAULT_KEY {
    ...
    static let HF_USER_SESSION = "HF_USER_SESSION"
    ...
    static let HF_ACCESS_TOKEN = "HF_ACCESS_TOKEN"
    static let HF_REFRESH_TOKEN = "HF_REFRESH_TOKEN"
}


struct NotificationName {
    static let shouldForcelogoutApp = "shouldForcelogoutApp"
}

public extension NSNotification.Name {
    static let shouldReloadRouteList = Notification.Name("shouldReloadRouteList")
    static let createStopSuccess = Notification.Name("createStopSuccess")
    static let updateStopSuccess = Notification.Name("updateStopSuccess")
    static let shouldForcelogoutApp = Notification.Name(NotificationName.shouldForcelogoutApp)
}
...
enum ProtocolURL: String {
    case http              = "http://"
    case https             = "https://"
}

func Loger(_ object: Any, functionName: String = #function, fileName: String = #file, lineNumber: Int = #line ) {
    #if DEBUG
    let className = (fileName as NSString).lastPathComponent
    print("<\(className)> \(functionName) [#\(lineNumber)]|\(Date().toDateString(dateFormater: "hh:mm:ss"))| \(object)\n")
    #endif
}
